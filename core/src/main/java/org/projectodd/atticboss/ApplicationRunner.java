...
package org.projectodd.atticboss;
...
public class ApplicationRunner {
    ...
    public void start(String[] args) throws Exception {
        ...
        AtticBoss.putOption("argv", args);
        AtticBoss.putOption("root", properties.getProperty("root"));
        String language = requiredProperty(properties, "language");
        log.info("Initializing " + name + " as " + language);
        AtticBoss.findLanguage(language)
    ...
    protected void copyConfigProperties() {
        for (String key : this.properties.stringPropertyNames()) {
            if (key.startsWith("config.")) {
                AtticBoss.putOption(key.substring(7), this.properties.getProperty(key));
            }
        }
    }
    ...
    protected void loadProperties() throws Exception {
        ...
        InputStream configStream = AtticBoss.classLoader().getResourceAsStream(internalPath);
    ...
    protected void extractJar() throws Exception {
        ...
        extractRoot = Files.createTempDirectory("atticboss").toFile().getAbsolutePath();
        ...
        AtticBoss.putOption("extract-root", extractRoot);
    }
    ...
    protected void updateClassPath() throws Exception {
        ...
        for (File file : classPathAdditions) {
            try {
                AtticBoss.updateClassPath(file.toURI().toURL());
    ...
    private static final Logger log = AtticBoss.logger("org.projectodd.atticboss");
}
