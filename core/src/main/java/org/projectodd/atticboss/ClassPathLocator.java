...
public class ClassPathLocator implements Locator {
    ...
    protected Class languageClassFor(String lang) {
        try {
            return loadClassFromResourceFile("META-INF/atticboss-languages/" + lang);
        } catch (Exception e) {
    ...
    protected Class<ComponentProvider> providerClassFor(Class clazz) {
        try {
            return (Class<ComponentProvider>)loadClassFromResourceFile("META-INF/atticboss-providers/" + clazz.getCanonicalName());
        } catch (Exception e) {
    ...
    private static final Logger log = AtticBoss.logger(ClassPathLocator.class);
}
